...
module.exports = async context => {
    ...
    if (false) {
        const [snapshotId] = await multiExecAsync(client, multi => {
            multi.hget('reo:h', 'snapshotId');
        });
    }
    while (true) {
        const [key, snapshotId] = await multiExecAsync(client, multi => {
            multi.brpoplpush('reo:key:q', 'reo:busy:key:q', 1);
        });
        if (!key) {
        ...
        try {
            ...
            const [modtimeString, psha] = await multiExecAsync(client, multi => {
                multi.hget(`reo:modtime:h`, key);
                multi.hget(`reo:sha:h`, key);
            });
            ...
            if (!content) {
                ...
                await multiExecAsync(client, multi => {
                    ...
                    multi.lrem('reo:busy:key:q', 1, key);
                    if (config.outq) {
